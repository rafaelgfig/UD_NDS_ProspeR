...
<p>A *MonthlyLoanPayment* mesmo omitido os valores mais altos ainda continua bastante assimétrica, ainda assim não achei necessário redimensioná-la, já que estava interessado em ver a concentração incomum dos dados próximos ao 175.</p>
...
ggplot(dff, aes(x = LoanOriginationDate, y = BorrowerAPR, color = factor(ProsperScore))) +
  ...
  theme_dark()
```
<br/>Nesse gráfico ficou claro os ajustes do sistema de pontuação. No começo houveram poucas pontuações baixas, mas com o passar do tempo ele foi ajustando, até mesmo ocorrendo saltos maiores de taxas. 
...
```{r Densidade LoanOriginalAmount StatedMonthlyIncome e EmploymentStatus, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Density LoanOriginalAmount, StatedMonthlyIncome e colorido pelo EmploymentStatus
ggplot(dff, aes(x = StatedMonthlyIncome, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(100, 1000, 2500, 5000, 10000, 15000, 20000)) -> g1
ggplot(dff, aes(x = LoanOriginalAmount, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(1000, 2500, 5000, 10000, 15000, 20000, 30000)) -> g2
grid.arrange(g1, g2, ncol = 1)
```
```{r Distribuicao LoanOriginalAmount StatedMonthlyIncome e EmploymentStatus, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Scatterplot LoanOriginalAmount, StatedMonthlyIncome e colorido pelo EmploymentStatus
ggplot(dff, aes(x = StatedMonthlyIncome, y = LoanOriginalAmount, color = EmploymentStatus)) +
  geom_point(alpha = 1/3) +
  theme_dark() +
  scale_y_continuous(trans = 'sqrt', breaks = c(1000, 2500, 5000, 10000, 15000, 20000, 30000)) +
  scale_x_continuous(trans = 'sqrt', breaks = c(100, 1000, 2500, 5000, 10000, 15000, 20000))
```
<br/>Intuitivamente o gráfico mostra - apesar de muito poluído - que mutuários normalmente possuem rendas altas e grandes empréstimos quando estão empregados, seja o tempo todo ou meio período. Ainda assim vemos que mutuários com baixa renda ainda conseguem realizar empréstimos relativamente alto, obtendo um gráfico de densidade mais uniforme do que em comparação com o das rendas.

```{r Facet LoanOriginalAmount StatedMonthlyIncome EmploymentStatus e Score, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
# Scatterplot LoanOriginalAmount, StatedMonthlyIncome e colorido pelo EmploymentStatus
ggplot(dff, aes(x = StatedMonthlyIncome, y = LoanOriginalAmount, color = EmploymentStatus)) +
  geom_point(alpha = 1/3) +
  theme_dark() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{r Distribuicao LoanDate LoanAmount e EmploymentStatus, echo=FALSE, message=FALSE, warning=FALSE, fig.height=10, fig.width=10}
# Scatterplot da data do empréstimo pelos valores com coloração do status do emprego
ggplot(dff, aes(x = LoanOriginationDate, y = LoanOriginalAmount, color = EmploymentStatus)) +
  geom_point(alpha = 1/2) +
  scale_x_date(breaks = '1 year', date_labels = '%Y') +
  theme_dark() -> g1
ggplot(subset(dff, !(EmploymentStatus %in% c('Employed','Full-time','','Not available'))), aes(x = LoanOriginationDate, y = LoanOriginalAmount, color = EmploymentStatus)) +
  geom_point(alpha = 1/2) +
  scale_x_date(breaks = '1 year', date_labels = '%Y', limits = c(min(dff$LoanOriginationDate), max(dff$LoanOriginationDate))) +
  theme_dark() -> g2
grid.arrange(g1, g2, ncol=1)
```
<br/>Com o gráfico dividido em score dá pra ver que existem muitos casos sem Score. Como já tinha visto anteriormente que o Score foi implementado após a lacuna, decidi ver como está distribuído os status do emprego ao longo do tempo. Para minha surpresa vi que houve período sem categoria, outro indisponível e depois o uso do termo "Full-time" que posteriormente foi substituido pelo "Employed". O termo "Other" também parece ter sido utilizado de fato só após 2011.

```{r Densidade LoanOriginalAmount StatedMonthlyIncome e ListingCategory, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Density LoanOriginalAmount, StatedMonthlyIncome e colorido pelo ListingCategory..numeric.
ggplot(dff, aes(x = StatedMonthlyIncome, colour = dff$ListingCategory..numeric.)) +
  geom_density(adjust = 1.5, size = .60, alpha = 1/2) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(100, 1000, 2500, 5000, 10000, 15000, 20000)) -> g1
ggplot(dff, aes(x = LoanOriginalAmount, colour = ListingCategory..numeric.)) +
  geom_density(adjust = 1.5, size = .60, alpha = 1/2) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(1000, 2500, 5000, 10000, 15000, 20000, 30000)) -> g2
grid.arrange(g1, g2, ncol = 1)
```
<br/>A linha 5 está mais a esquerda do que as demais, o que faz sentido uma vez que corresponda aos empréstimos dos estudantes - anteriormente verificados como menores rendas -, os valores dos empréstimos dessa categoria também estão concentrados nos valores mais baixos, não seguindo a tendência das outras categorias. Mais uma vez vemos a tendência dos dados demonstrando que vida de estudante não é fácil.<br/>
...
```{r Boxplot APR StatedMonthlyIncome.bucket e Score, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Boxplot APR, StatedMonthlyIncome e colorido pelo Score
ggplot(subset(dff, !is.na(StatedMonthlyIncome.bucket) & !is.na(ProsperScore) ), aes(x = factor(ProsperScore), y = BorrowerAPR, fill = factor(ProsperScore))) +
  geom_boxplot() +
  scale_fill_manual(values = cc) +
  facet_wrap(~StatedMonthlyIncome.bucket)
```
```{r Boxplot LoanOriginalAmount StatedMonthlyIncome.bucket e Score, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Boxplot LoanOriginalAmount, StatedMonthlyIncome e colorido pelo Score
ggplot(subset(dff, !is.na(StatedMonthlyIncome.bucket) & !is.na(ProsperScore) ), aes(x = factor(ProsperScore), y = LoanOriginalAmount, fill = factor(ProsperScore))) +
  geom_boxplot() +
  scale_fill_manual(values = cc) +
  facet_wrap(~StatedMonthlyIncome.bucket)
```
<br/>Esse ficou interessante, pois as taxas pelo score seguem a mesma tendência ao longo das faixas de rendas, mas isso não ocorre com o valor do empréstimo. Por mais que já tenha sido visto que a renda tenha correlação com o valor solicitado, não esperava que o score tivesse essa tendência nas rendas de até 2.500.<br/>
...
### Primeiro Gráfico
```{r Primeiro Grafico, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10}
# Valores dos buckets por extenso
labs = c('$0  até  $2,499', '$2,500  até  $4,999', '$5,000  até  $7,499', '$7,500  até  $9,999',
         '$10,000  até  $12,499', '$12,500  até  $14,999', '$15,000  até  $17,499', '$17,500  até  $19,999',
         '$20,000  até  $22,499', '$22,500  até  $24,499')
dff$StatedMonthlyIncome.bucket.labs <- factor(dff$StatedMonthlyIncome.bucket, labels = labs)
# Boxplot dos valores de empréstimos divididos entre os Scores e fatiado pelas faixas de renda
ggplot(subset(dff, !is.na(StatedMonthlyIncome.bucket) & !is.na(ProsperScore) ), aes(x = factor(ProsperScore), y = LoanOriginalAmount, fill = factor(ProsperScore))) +
  geom_boxplot() +
  scale_fill_manual(values = cc) +
  scale_y_continuous(breaks = seq(0, 35000, 5000)) +
  geom_hline(yintercept = seq(0, 35000, 5000), alpha = 1/3, color = 'white') +
  geom_hline(yintercept = 4000, alpha = 1/3) +
  facet_wrap(~StatedMonthlyIncome.bucket.labs) +
    labs(title = 'Valores Origináis dos Empréstimos sobre os Scores dos Mutuários nas Faixas de Renda', 
         x = 'Score', 
         y = 'Valor (em dólares)') +
...
### Segundo Gráfico
```{r Segundo Grafico, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10}
# Scatterplot da data do empréstimo pelos valores com coloração do score
cc <- scales::seq_gradient_pal('red','green')(seq(0,1, length.out = 11))
ggplot(dff, aes(x = LoanOriginationDate, y = LoanOriginalAmount, color = factor(ProsperScore))) +
  geom_point(alpha = 1/2) +
  ...
  scale_color_manual(values = cc, na.value = 'dark gray') +
  theme_dark() +
    labs(title = 'Valores Origináis dos Empréstimos pela Data em que foi Realizado e colorido pelo Score', 
         x = 'Data', 
         y = 'Valor (em dólares)',
         color = 'Score') +
  theme(plot.title = element_text(hjust = 0.5)) -> g21
# Scatterplot da data do empréstimo pelas taxas com coloração do score
ggplot(dff, aes(x = LoanOriginationDate, y = BorrowerAPR, color = factor(ProsperScore))) +
  geom_point(alpha = 1/2) +
  scale_x_date(breaks = '1 year', date_labels = '%Y') +
  scale_color_manual(values = cc, na.value = 'dark gray') +
  theme_dark() +
    labs(title = 'Taxa APR do Mutuário pela Data em que foi Realizado e colorido pelo Score', 
         x = 'Data', 
         y = 'Taxa',
         color = 'Score') +
  theme(plot.title = element_text(hjust = 0.5)) -> g22
...
### Terceiro Gráfico
```{r Terceiro Grafico, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
# Densityplot StatedMonthlyIncome e colorido pelo EmploymentStatus
ggplot(subset(dff, EmploymentStatus != ''), aes(x = StatedMonthlyIncome, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(100, 1000, 2500, 5000, 10000, 15000, 20000)) +
    labs(title = 'Densidades das Rendas coloridas pelo Status do Emprego', 
         x = 'Renda (em dólares)', 
         y = 'Densidade',
         color = 'Status') +
  theme(plot.title = element_text(hjust = 0.5)) -> g31
# Densityplot LoanOriginalAmount e colorido pelo EmploymentStatus
ggplot(subset(dff, EmploymentStatus != ''), aes(x = LoanOriginalAmount, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(1000, 2500, 5000, 10000, 15000, 20000, 30000)) +
    labs(title = 'Densidades dos Valores Origináis dos Empréstimos coloridas pelo Status do Emprego', 
         x = 'Valor (em dólares)', 
         y = 'Densidade',
         color = 'Status') +
  theme(plot.title = element_text(hjust = 0.5)) -> g32
...
Selecionei as colunas que achei mais interessantes para tentar entender os perfis das pessoas que solicitam empréstimos, mas ao longo da análise algunas delas não foram utilizadas. <br/>
