...
ggplot(dff, aes(x = LoanOriginationDate, y = BorrowerAPR, color = factor(ProsperScore))) +
  ...
  theme_dark()
```
<br/>Nesse gráfico ficou claro os ajustes do sistema de pontuação. No começo houveram poucas pontuações baixas, com o passar do tempo ele foi ajustando, até mesmo ocorrendo saltos maiores de taxas, um exemplo disso são as linhas horizontais na mesma cor de Score, esse provavelmente foi o período que mais se basearam no score para estimar as taxas. <br/>
...
### Primeiro Gráfico
```{r Primeiro Grafico, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
ggplot(dff, aes(x = StatedMonthlyIncome, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(100, 1000, 2500, 5000, 10000, 15000, 20000)) -> g31
ggplot(dff, aes(x = LoanOriginalAmount, colour = EmploymentStatus)) +
  geom_density(adjust = 2, size = .65) +
  theme_dark() +
  scale_x_continuous(trans = 'sqrt', breaks = c(1000, 2500, 5000, 10000, 15000, 20000, 30000)) -> g32
...
Os dados são limitados ao ano de 2014, então atualmente (2019) o sistema de Score deve ter muito mais peso do que antes, pois houve um claro ajuste desse sistema. Olhar para as variáveis que compõe o Score certamente ajudaria entender os perfís dos mutários e isso poderia talvez auxiliar ma criação de propostas ou propagandas direcionadas.
